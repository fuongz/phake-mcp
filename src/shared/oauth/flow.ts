...
export async function handleProviderCallback(
	...
	const decodedObj = base64UrlDecodeJson<{
		tid?: string;
		cs?: string;
		cr?: string;
		sid?: string;
	...
	if (decodedObj) {
		...
		// State is not JSON — treat as raw txnId (opaque token fallback)
		logger.debug("oauth_callback", {
	...
	// Build callback URL params and validate via oauth4webapi (required by authorizationCodeGrantRequest)
	...
	try {
		...
		// txn.clientRedirectUri was stored server-side at authorize time — trust it directly.
		// decoded.cr (from state) is untrusted, so still goes through the allowlist check.
		const safe =
			txn.clientRedirectUri ??
			(decoded.cr && isAllowedRedirect(decoded.cr, oauthConfig, options.isDev)
				? decoded.cr
				: oauthConfig.redirectUri);
