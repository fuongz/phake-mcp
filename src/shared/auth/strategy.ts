 ...
 */
export function parseAuthStrategy(
...
export function buildAuthHeaders(
	...
	switch (strategyConfig.type) {
		...
		case "oauth":
		case "none":
			// OAuth/Google/GitHub headers are resolved dynamically via RS token mapping
			// 'none' has no headers
			break;
	}
 ...
 */
export function resolveStaticAuth(
	strategyConfig: AuthStrategyConfig,
): ResolvedAuth {
...
export function validateAuthConfig(config: AuthStrategyConfig): string[] {
	...
	switch (config.type) {
		...
		case "oauth":
			if (!config.value) {
				errors.push(
					"OAUTH_CLIENT_ID is required when AUTH_STRATEGY=google|github|oauth",
				);
			}
			break;
	}
